...
public static class AudioSourceExtend
{
    ...
    /// 获取音频片段的总时间
    /// </summary>
    public static string GetAudioClipTotalTime( this AudioClip clip )
    {
    ...
    /// <returns></returns>
    public static string GetSoundCurrentTime( this AudioSource source )
    {
...
public static class JsonExtend
{
    ...
    public static void WriterJson<T>( T json , string savePath )
    {
        if( json == null )
        {
        ...
        {
            throw new Exception( "json file save path is null" );
        }
    ...
    /// <returns>解析后得json数据</returns>
    public static T AnalysisJson<T>( string path )
    {
...
public static class UnityEngineExtend
{
    ...
    /// <summary>
    /// 创建物体使用默认Transform信息并设置父物体,如果为父物体则不设置
    /// </summary>
    /// <param name="parent">父物体，如果为空则不设置父物体</param>
    /// <param name="name">物体名</param>
    /// <param name="localPost">使用本地坐标还是世界坐标</param>
    /// <param name="components">需要添加的组件</param>
    /// <returns></returns>
    public static GameObject CreationObjectSetInfo( string name = "GameObject" , bool localPost = true , Transform parent = null , params Type[] components )
    {
...
public static class VideoExtend
{
    ...
    /// <returns></returns>
    public static string GetVideoClipTotalTime( this VideoClip clip )
    {
        int hour = (int)clip.length / 3600;
        int minute = (int)( clip.length - hour * 3600 ) / 60;
        int second = (int)( clip.length - hour * 3600 - minute * 60 );
    ...
    /// <returns></returns>
    public static string GetVideoPlayCurrentTime( this VideoPlayer player )
    {
        int hour = (int)player.time / 3600;
        int minute = (int)( player.time - hour * 3600 ) / 60;
        int second = (int)( player.time - hour * 3600 - minute * 60 );
